...
namespace DrinkPsych.Models
{
    public class ShoppingCart
    {
        ...
        public static ShoppingCart GetCart(IServiceProvider services)
        {
            ...
            string cartId = session.GetString("CartId") ?? Guid.NewGuid().ToString(); //checing session for cartid
        ...
        public decimal GetShoppingCartTotal()
        {
            var total = _appDbContext.ShoppingCartItems.Where(
                c => c.ShoppingCartId == ShoppingCartId).Select(
                c => c.Drink.Price * c.Amount).Sum();
